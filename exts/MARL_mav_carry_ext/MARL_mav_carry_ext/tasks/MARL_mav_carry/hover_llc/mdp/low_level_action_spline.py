    ...
    def apply_actions(self):
        ...
        if self._ll_counter % self.cfg.low_level_decimation == 0:
            ...
            for i in range(self._num_drones):
                ...
                drone_end_state = torch.cat(
                    [drone_waypoints, desired_vel_end, desired_acc_end, desired_jerk_end, desired_snap_end], dim=1
                )
                ...
                # generate spline
                spline_coeffs = septic_spline.get_coeffs(
                    first_waypoint, drone_end_state, self._times * self._time_horizon, self.num_envs
                )
                # get individual rotor thrusts from geometric controller
                position, velocity, acceleration, jerk, snap = septic_spline.evaluate_trajectory(
