...
class CTBR(ActionTerm):
    ...
    def __init__(self, cfg: LowLevelActionCfg, env: ManagerBasedRLEnv):
        ...
        # buffers
        self.action = torch.zeros(self.num_envs, self._num_drones * 4, device=self.device)
        ...
        self.sampling_time = self._env.sim.get_rendering_dt()
    ...
    def process_actions(self, action: torch.Tensor):
        """Process the actions and apply the forces and torques to the drone bodies.
        Args:
            action: The policy output to be processed.
        """
        self.action = action
    ...
    def apply_actions(self):
        ...
        for i in range(self._num_drones):
            start_drone_idx = int(i * self.action_dim / self._num_drones)
            end_drone_idx = int((i + 1) * self.action_dim / self._num_drones)
            drone_actions = self.action[:, start_drone_idx:end_drone_idx]
            thrusts = self.G_1_inv.matmul(drone_actions.transpose(0, 1)).transpose(0, 1)
            thrusts = torch.clamp(thrusts, 0, 6.25) # TODO: change to actual min and max thrust
            target_rate = torch.sqrt(thrusts / self._motor_model.thrust_map[0])
...
@configclass
class CTBRCfg(ActionTermCfg):
    """Configuration for the low level action term."""

    class_type: type[ActionTerm] = CTBR
