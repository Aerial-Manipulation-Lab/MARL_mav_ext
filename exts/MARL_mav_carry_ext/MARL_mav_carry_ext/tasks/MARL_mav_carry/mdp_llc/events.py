...
def reset_root_state_uniform_collision_check(
    ...
    * It samples the root position from the given ranges and adds them to the default root position, before setting
      them into the physics simulation.
    * It then checks if the asset collides with any other asset in the scene. If it does, it resamples the root
      position and repeats the process until the asset does not collide.
    * It samples the root orientation from the given ranges and sets them into the physics simulation.
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    range_list = [pose_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
    ranges = torch.tensor(range_list, device=asset.device)
    # Initialize rand_samples and positions
    rand_samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=asset.device)
    positions = root_states[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3]

    # Collision check and resampling
    collision_threshold = 1.5  # meters
    in_collision = torch.ones(len(env_ids), device=asset.device, dtype=torch.bool)  # Start with all envs in collision
