...
class LowLevelAction_traj(ActionTerm):
    """Low level action term for the hover task."""

    cfg: LowLevelActionCfg_traj

    def __init__(self, cfg: LowLevelActionCfg_traj, env: ManagerBasedRLEnv):
    ...
    def process_actions(self, waypoints: torch.Tensor):
        ...
        if self.cfg.is_trajectory:
            ...
            self._eval_time = (self._sim_dt/self._time_horizon) / (self.cfg.planner_decimation/self.cfg.low_level_decimation) 
            # normalized planner interval, divided by how many times LLC runs per planner call
            ...
            for i in range(self._num_drones):
                ...
                first_waypoint = torch.cat(
                        (drone_states["pos"], drone_states["lin_vel"], drone_states["lin_acc"], drone_states["jerk"]), dim=-1
                    )
                ...
                current_drone = f"drone_{i}"
                self._spline_coeffs[current_drone] = spline_coeffs
    ...
    def apply_actions(self):
        ...
        if self._ll_counter % self.cfg.low_level_decimation == 0:
            ...
            for i in range(self._num_drones):
                ...
                if self.cfg.is_trajectory:
                    current_drone = f"drone_{i}"
                    spline_coeffs = self._spline_coeffs[current_drone]
                    pos, vel, acc, jerk, snap = quintic_spline.evaluate_trajectory(spline_coeffs, self._times * self._time_horizon, self._eval_time * self._time_horizon)
                    drone_setpoint["pos"] = pos
                    drone_setpoint["lin_vel"] = vel
                    drone_setpoint["lin_acc"] = acc
    ...
    def _debug_vis_callback(self, event):
        ...
        if self.cfg.is_trajectory:
            ...
            for i in range(self._num_drones):
                ...
                spline_coeffs = self._spline_coeffs[current_drone]
                pos_list = []
                eval_times = torch.linspace(0, self._time_horizon, 100)
                for i in eval_times:
                    pos, vel, acc, jerk, snap = quintic_spline.evaluate_trajectory(spline_coeffs, self._times * self._time_horizon, i)
                    pos_list.append(pos.flatten().tolist())
                    # pos_list.append(pos[0].cpu())
                self.draw_interface.draw_lines_spline(
                    pos_list,
                    (1.0, 1.0, 1.0, 1.0),
                    3,
                    False
...
@configclass
class LowLevelActionCfg_traj(ActionTermCfg):
    """Configuration for the low level action term."""

    class_type: type[ActionTerm] = LowLevelAction_traj
    ...
    """Whether the waypoints are a trajectory or a single waypoint."""
    num_waypoints: int = 1
    """Number of waypoints in the trajectory."""
    time_horizon: float = 2.0
    """Time horizon of the trajectory in seconds."""
    planner_decimation: int = 10
    """Decimation factor for the planner."""
