...
class LowLevelAction(ActionTerm):
    ...
    def process_actions(self, waypoints: torch.Tensor):
        ...
        for i in range(self._num_drones):
            ...
            if self._delta_output:
                self.drone_setpoint[i]["pos"] = self.previous_setpoint[:, i, :3] + drone_waypoints[:, :3]
                self.drone_setpoint[i]["lin_vel"] = self.previous_setpoint[:, i, 3:6] + drone_waypoints[:, 3:6]
                self.drone_setpoint[i]["lin_acc"] = self.previous_setpoint[:, i, 6:9] + drone_waypoints[:, 6:9]
                self.drone_setpoint[i]["jerk"] = self.previous_setpoint[:, i, 9:12] + drone_waypoints[:, 9:12]
