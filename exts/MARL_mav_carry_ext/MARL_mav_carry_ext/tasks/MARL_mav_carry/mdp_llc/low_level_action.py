...
class LowLevelAction(ActionTerm):
    ...
    def __init__(self, cfg: LowLevelActionCfg, env: ManagerBasedRLEnv):
        ...
        self.drone_setpoint = [{}, {}, {}]
        self.spline_positions = torch.zeros(self.num_envs, 3, 6, 3, device=self.device) # 5 previous positions and policy ouptut
    ...
    def process_actions(self, waypoints: torch.Tensor):
        ...
        for i in range(self._num_drones):
            ...
            if self._spline: # TO ADD FOR SPLINE: history of pos, enable in hover env, add tanh, change waypoint_dim
                # if using real past positions
                self.spline_positions[:, i, -1] = self.drone_positions[:, i] 
                # update buffer
                self.spline_positions = update_buffer(self.spline_positions, i, self.drone_positions[:, i] + drone_waypoints[:, :3] * 0.1)

                self.spline_coeffs[:, i] = quintic_trajectory_coeffs(self.spline_positions[:, i], self._time_horizon, self.num_envs)
                self._desired_position[:, i] = self.drone_positions[:, i] + drone_waypoints[:, :3]

            # send waypoint generated by the policy
            if self._control_mode == "geometric" and not self._spline:
    ...
    def apply_actions(self):
        ...
        if self._ll_counter % self.cfg.low_level_decimation == 0:
            ...
            for i in range(self._num_drones):
                ...
                drone_states["pos"] = self.drone_positions.view(self.num_envs, -1)[:, i * 3 : i * 3 + 3]
                drone_states["quat"] = self.drone_orientations.view(self.num_envs, -1)[:, i * 4 : i * 4 + 4]
                drone_states["lin_vel"] = self.drone_linear_velocities.view(self.num_envs, -1)[:, i * 3 : i * 3 + 3]
                drone_states["ang_vel"] = self.drone_angular_velocities.view(self.num_envs, -1)[:, i * 3 : i * 3 + 3]
                drone_states["lin_acc"] = self.drone_linear_accelerations.view(self.num_envs, -1)[:, i * 3 : i * 3 + 3]
                drone_states["ang_acc"] = self.drone_angular_accelerations.view(self.num_envs, -1)[:, i * 3 : i * 3 + 3]
                ...
                if self._spline:
                    ...
                    self.drone_setpoint[i]["lin_acc"] = acc
                    self.drone_setpoint[i]["jerk"] = jerk
...
class LowLevelActionCfg(ActionTermCfg):
    ...
    spline: bool = False
    """Whether to use spline for trajectory generation. Defaults to False."""