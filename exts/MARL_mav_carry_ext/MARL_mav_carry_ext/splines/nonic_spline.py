...
def compute_derivatives(coeffs, power):
    if power == 1:
        return coeffs[:, :-1] * torch.tensor(
            [9, 8, 7, 6, 5, 4, 3, 2, 1], dtype=torch.float32, device=torch.device("cuda")
        )
    elif power == 2:
        return coeffs[:, :-2] * torch.tensor(
            [72, 56, 42, 30, 20, 12, 6, 2], dtype=torch.float32, device=torch.device("cuda")
        )
    elif power == 3:
        return coeffs[:, :-3] * torch.tensor(
            [504, 336, 210, 120, 60, 24, 6], dtype=torch.float32, device=torch.device("cuda")
        )
    elif power == 4:
        return coeffs[:, :-4] * torch.tensor(
            [3024, 1680, 840, 360, 120, 24], dtype=torch.float32, device=torch.device("cuda")
        )
    else:
        return coeffs
...
def get_coeffs(waypoints, times, num_envs):
    ...
    for i in range(n_points - 1):
        ...
        start = {
            ...
            "snap": waypoints[:, curr_point_id + 12 : curr_point_id + 15],
        }
        next_point_id = (i + 1) * waypoint_id_increment
            ...
            "snap": waypoints[:, next_point_id + 12 : next_point_id + 15],
        }
        t_start = times[i]
        t_end = times[i + 1]
    ...
    for i in range(len(times) - 1):
        ...
        if t_start <= t_eval <= t_end:
            ...
            velocity_x = torch.sum(
                ...
                * torch.tensor(
                    [T**8, T**7, T**6, T**5, T**4, T**3, T**2, T, 1], dtype=torch.float32, device=torch.device("cuda")
                ),
                dim=-1,
                keepdim=True,
            )
