...
def quintic_trajectory(start, end, t_start, t_end):
    ...
    b_x = torch.tensor([start['pos'][0], end['pos'][0], start['vel'][0], end['vel'][0], start['acc'][0], end['acc'][0]], dtype=torch.float32, device=torch.device('cuda'))
    b_y = torch.tensor([start['pos'][1], end['pos'][1], start['vel'][1], end['vel'][1], start['acc'][1], end['acc'][1]], dtype=torch.float32, device=torch.device('cuda'))
    b_z = torch.tensor([start['pos'][2], end['pos'][2], start['vel'][2], end['vel'][2], start['acc'][2], end['acc'][2]], dtype=torch.float32, device=torch.device('cuda'))
...
def evaluate_trajectory(coeffs_list, times, t_eval):
    for i in range(len(times) - 1):
        ...
        if t_start <= t_eval <= t_end:
            ...
            position_x = torch.sum(coeffs_x * torch.tensor([T**5, T**4, T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            position_y = torch.sum(coeffs_y * torch.tensor([T**5, T**4, T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            position_z = torch.sum(coeffs_z * torch.tensor([T**5, T**4, T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            position = torch.tensor([position_x, position_y, position_z], dtype=torch.float32, device=torch.device('cuda'))
            
            # Compute velocity
            velocity_x = torch.sum(compute_derivatives(coeffs_x, 1) * torch.tensor([T**4, T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            velocity_y = torch.sum(compute_derivatives(coeffs_y, 1) * torch.tensor([T**4, T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            velocity_z = torch.sum(compute_derivatives(coeffs_z, 1) * torch.tensor([T**4, T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            velocity = torch.tensor([velocity_x, velocity_y, velocity_z], dtype=torch.float32, device=torch.device('cuda'))
            
            # Compute acceleration
            acceleration_x = torch.sum(compute_derivatives(coeffs_x, 2) * torch.tensor([T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            acceleration_y = torch.sum(compute_derivatives(coeffs_y, 2) * torch.tensor([T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            acceleration_z = torch.sum(compute_derivatives(coeffs_z, 2) * torch.tensor([T**3, T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            acceleration = torch.tensor([acceleration_x, acceleration_y, acceleration_z], dtype=torch.float32, device=torch.device('cuda'))
            
            # Compute jerk
            jerk_x = torch.sum(compute_derivatives(coeffs_x, 3) * torch.tensor([T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            jerk_y = torch.sum(compute_derivatives(coeffs_y, 3) * torch.tensor([T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            jerk_z = torch.sum(compute_derivatives(coeffs_z, 3) * torch.tensor([T**2, T, 1], dtype=torch.float32, device=torch.device('cuda')))
            jerk = torch.tensor([jerk_x, jerk_y, jerk_z], dtype=torch.float32, device=torch.device('cuda'))
            
            # Compute snap
            snap_x = torch.sum(compute_derivatives(coeffs_x, 4) * torch.tensor([T, 1], dtype=torch.float32, device=torch.device('cuda')))
            snap_y = torch.sum(compute_derivatives(coeffs_y, 4) * torch.tensor([T, 1], dtype=torch.float32, device=torch.device('cuda')))
            snap_z = torch.sum(compute_derivatives(coeffs_z, 4) * torch.tensor([T, 1], dtype=torch.float32, device=torch.device('cuda')))
