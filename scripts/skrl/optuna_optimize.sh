...
for ((i=1; i<=NUM_TRIALS; i++)); do
    ...
    SUGGESTED_VALUES=$(optuna ask --storage $STORAGE_URL --study-name $STUDY_NAME --sampler TPESampler \
        --search-space '{
            "learning_rate_actor": {"name": "FloatDistribution", "attributes": {"step": null, "low": 0.00001, "high": 0.1, "log": true}},
            "learning_rate_critic": {"name": "FloatDistribution", "attributes": {"step": null, "low": 0.00001, "high": 0.1, "log": true}},
            "epochs": {"name": "IntDistribution", "attributes": {"low": 1, "high": 5}},
            "lambda": {"name": "FloatDistribution", "attributes": {"step": null, "low": 0.2, "high": 1.0}}
        }')

    # Extract the trial parameters
    LEARNING_RATE_ACTOR=$(echo $SUGGESTED_VALUES | jq -r '.params.learning_rate_actor')
    LEARNING_RATE_CRITIC=$(echo $SUGGESTED_VALUES | jq -r '.params.learning_rate_critic')
    EPOCHS=$(echo $SUGGESTED_VALUES | jq -r '.params.epochs')
    ...
    TRIAL_OUTPUT=$(python3 scripts/skrl/hyper_param_search.py \
        --task $TASK \
        --suggested_lr_actor $LEARNING_RATE_ACTOR \
        --suggested_lr_critic $LEARNING_RATE_CRITIC \
        --suggested_learning_epochs $EPOCHS \
        --optuna_id $TRIAL_ID \
